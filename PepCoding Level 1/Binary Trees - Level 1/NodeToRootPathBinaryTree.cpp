// Question Link: https://nados.io/question/find-and-nodetorootpath-in-binary-tree
// Video Link: https://youtu.be/1Kyc-zQS7eQ?list=PLNQqARTgXkMbp1R47hg9NnCNVAraXJv99
// Analysis: https://nados.io/article/find-node-to-root-path-in-binary-tree#:~:text=4.-,Complexities,-%3A
#include <bits/stdc++.h>
using namespace std;
...
vector<int> nodeToRootPath(TreeNode* root, int val) {
    ...
    vector<int> pathFromLeft{nodeToRootPath(root->left, val)};
    vector<int> pathFromRight{nodeToRootPath(root->right, val)};

    if (pathFromLeft.empty() and not pathFromRight.empty()) {
        pathFromRight.emplace_back(root->val);
        return pathFromRight;
    }

    if (pathFromRight.empty() and not pathFromLeft.empty()) {
        pathFromLeft.emplace_back(root->val);
        return pathFromLeft;
    }
...
int main() {
    ...
    cout << "Node to root path for given value: ";
    for (auto&& val : nodeToRootPath(root, value))
        cout << val << " ";
    cout << "\n\n";
