// Question Link: https://www.pepcoding.com/resources/online-java-foundation/dynamic-programming-and-greedy/coin-change-combination-official/ojquestion
// Video Link: https://youtu.be/l_nR5X9VmaI?list=PL-Jc9J83PIiG8fE6rj9F5a6uyQ5WPdqKy
...
class CoinChangeCombination {
	...
	private static int memoize(final int[] coins, int amount, int index, Integer[][] memo){
		...
		if(coins[index - 1] > amount)
			memo[index][amount] = memoize(coins, amount, index - 1, memo);
		else
			memo[index][amount] = memoize(coins, amount, index - 1, memo) + memoize(coins, amount - coins[index - 1], index, memo);
