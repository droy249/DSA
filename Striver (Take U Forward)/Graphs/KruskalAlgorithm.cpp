...
// Video Link: https://youtu.be/1KRmCzBl_mQ?list=PLgUwDviBIf0rGEWe64KWas0Nryn7SCRWw
// Complexity Analysis: https://takeuforward.org/data-structure/minimum-spanning-tree-mst-using-kruskals-algo/#:~:text=Time-,Complexity,-%3A%20O(ElogE
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    sort(edges.begin(), edges.end(), [](const node &a, const node &b) { return a.wt < b.wt; });
    ...
    cout << "\nFinal total cost of MST: " << cost << endl;
    for (const auto& it : mst) cout << it.first << " - " << it.second << endl;
}